...
export default function Map03({ components = [], sectionId = null }) {
  ...
  return ( <section className="py-20 px-6 bg-orange-50 border-y-4 border-orange-500"> <div className="pointer-events-none absolute top-4 right-6 text-[11px] font-bold bg-slate-900 text-white rounded-full px-4 py-2">FLEET LIVE</div> <div className="relative max-w-6xl mx-auto"> <div className="flex flex-col items-center text-center gap-1"> {renderLayoutComponents(layoutComponents.filter((c) => c.type !== 'image'), sectionId)} </div> <div className="mt-8 rounded-2xl border overflow-hidden select-none pointer-events-none" style={{ borderColor: '#fed7aa', height: '400px' }}> <div className="w-full h-full flex items-center justify-center" style={{ background: '#ffffff' }}> <div className="text-center"> <div className="text-6xl mb-4">PIN</div> <div className="text-lg font-bold">Map Location</div> <div className="text-sm">123 Main Street, City</div> </div> </div> </div> </div> </section> );
}
